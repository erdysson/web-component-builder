...
export const Module = (config: IModuleConfig): ClassDecorator => (target: IClass) => Metadata.setModuleConfig(target, config);

export const Component = (config: IComponentConfig): ClassDecorator => (target: IClass) => Metadata.setComponentConfig(target, config);
...
export const Inject = (providerClass: IClass): ParameterDecorator =>
    ...
    // @ts-ignore
    (target: IClass, propertyKey: string | symbol, parameterIndex: number) => Metadata.setInjectedProviderConfig(target, providerClass, parameterIndex);
